...
//
// **The rule used to be "nothing", and sdk#8 replaced it with this rather
...
// collector client. Those are what a *binary* wires (sdk#8), and a module
// that could reach them could configure the Platform's observability plane from
// inside it — the ownership sdk#5 placed with the Platform and sdk#8 did
// not move. Twelve modules come with the OTel SDK against four with the API,
// and the difference is exactly this boundary.
//
// The metric API joined the three in sdk#9, on the same terms: `metric.Meter`
...
// would notice, and whose own dependency list is the temptation: platform#39 split
